...
<%
String template = "<action id=\"%s\" uid=\"%s\" name=\"%s\" l2=\"%s\" location=\"%s\"%s/>";
String s;
for(Action action: list){
	...
	s = String.format(template, action.getId(), action.getUid(), action.getName(), action.getLocation(), action.getLocation(), info);
