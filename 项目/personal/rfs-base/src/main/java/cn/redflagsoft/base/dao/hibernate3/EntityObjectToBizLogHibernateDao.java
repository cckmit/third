/*
 * $Id: EntityObjectToBizLogHibernateDao.java 5147 2011-11-30 09:38:37Z lcj $
 * 
...
import cn.redflagsoft.base.bean.EntityObjectToBizLog;

@Deprecated
public abstract class EntityObjectToBizLogHibernateDao<K extends EntityObjectToBizLog> extends AbstractBaseHibernateDao<K, Long>{
	private Class<K> entityClass;
	
	public EntityObjectToBizLogHibernateDao(){
	...
	protected Class<K> determineEntityClass(){
	    ...
	    if(genType != null && genType instanceof ParameterizedType){
	    	...
	    	if(params.length >= 1){
		        ...
		        if(EntityObjectToBizLog.class.isAssignableFrom(cls)){
        			return (Class<K>) cls;
        		}
	    	}
	    }
	...
	public void setIdGeneratorProvider(IdGeneratorProvider<Long> idGeneratorProvider) {
		...
		setIdGenerator(idGeneratorProvider.getIdGenerator("OBJECT_TO_BIZLOG"));
	}
}
