/*
 * $Id: Order.java 13 2010-11-26 05:04:02Z alex $
 *
...
public class Order implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8590935420826449261L;
    ...
    private void checkOrderPropertyName(Order o, String propertyName){
	if(o.propertyName.equalsIgnoreCase(propertyName)){
	     throw new IllegalArgumentException("排序的属性名称重复: " + propertyName);
	}
    }
	...
	order.root = root;
	Order current = root;
	String propertyName = order.propertyName;
	checkOrderPropertyName(current, propertyName);
	...
	StringBuffer result = new StringBuffer(order.toOrderString());
        while (order.next != null) {
            order = order.next;
            //result.append(", " + order.toString());
	    result.append(", " + order.toOrderString());
	}
        return result.toString();
    }


    private String rootToString(boolean encoded) {
        if (encoded) {
