/*
 * $Id: Paginator.java 13 2010-11-26 05:04:02Z alex $
 *
...
public class Paginator {
	...
	public Paginator(int startIndex, int pageSize, int itemCount) {
		this(new DefaultPageable(startIndex, pageSize, itemCount));
	}
	...
	public Paginator(Pageable pageable) {
		...
		pageCount = (int) Math.ceil((double) pageable.getItemCount() / (double) pageable.getPageSize());
	...
	public int getPageIndex() {
		/*
		 * if (pageable.getPageSize() == 0) { return 0; } else { return
		 * pageable.getStartIndex() / pageable.getPageSize(); }
		 */
		return pageIndex;
	}
	...
	public Page[] getPages(int numViewablePages) {
		if (pages == null) {
			...
			int maxPages = pageCount >= numViewablePages ? numViewablePages : pageCount;
			List<Page> pageList = new ArrayList<Page>(maxPages);
			int i = 0;
			do {
				if (i >= maxPages) {
					break;
			...
			if (pageList.size() > 0) {
				Page lowestPage;
				Page page;
				for (lowestPage = (Page) pageList.get(0); pageList.size() < maxPages && lowestPage.getStart() != 0; lowestPage = page) {
