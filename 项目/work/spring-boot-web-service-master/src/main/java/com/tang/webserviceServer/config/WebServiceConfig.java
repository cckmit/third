...
import com.tang.webserviceServer.service.ProxyWebService;
import com.tang.webserviceServer.service.ServerServiceDemo;
import com.tang.webserviceServer.service.TestService;
...
public class WebServiceConfig {
    ...
    @Autowired
    private SignOutInterceptor signOutInterceptor;

    @Autowired
    private ProxyWebService proxyWebService;
    /**
   ...
   /* @Bean
    ...
    public Endpoint endpoint() {
        ...
        endpoint.getInInterceptors().add(signInterceptor);
        endpoint.getOutInterceptors().add(signOutInterceptor);
        return endpoint;
    }
    ...
    public Endpoint endpointTest() {
        ...
        endpoint.publish("/ws/test/api");
        return endpoint;
    }

    @Bean
    public Endpoint endpointProxy() {
        EndpointImpl endpoint = new EndpointImpl(springBus(), proxyWebService);
        endpoint.publish("/ws/proxy/api");
        endpoint.getInInterceptors().add(signInterceptor);
        endpoint.getOutInterceptors().add(signOutInterceptor);
        return endpoint;
    }
